...
const createApp = () => {
  ...
  app.use(
    cors({
      origin: clientOrigin, // This line of code specifies the allowed origin for cross-origin requests.
  ...
  // Health API - GET /health - quick check that the Express app is running
...
const startServer = async ({ port: customPort } = {}) => {
  if (serverInstance) {
...
if (require.main === module) {
  startServer()
    .then((server) => {
      ...
      console.log(`Server is successfully listening on port ${address.port}...`);
    })
    .catch((err) => {
